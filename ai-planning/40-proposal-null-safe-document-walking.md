...
**Status:** ✅ Implemented, except §4.5 (declined -- see §8)
...
Not yet actioned as of §8 -- implementation was authorised and completed; closing PRs on GitHub is a separate, outward-facing action that hasn't been requested yet.

## 8. What was actually built

Robert accepted the recommendations except §4.5 (the upstream fix -- declined, not pursued). Everything else in §3/§4 shipped, on branch `issue/92-null-safe-reference-walking`, test-first: the 16-case regression suite in `null-safety.test.ts` was written and confirmed to fail (by throwing an uncaught exception out of `merge()`, a legitimate red state) before any production code changed. Four things turned out differently from the design as written above, all found during implementation rather than assumed away:

1. **Pointer threading (§4.3 Option B) uses an *optional* `pointer?: string` parameter, not a required one.** ~30 existing tests in `references.test.ts` call `walkSchemaReferences`/`walkParameterReferences`/etc. directly with no pointer argument. A required parameter would have broken every one of them for no benefit -- they don't exercise malformed input. Every walk function's pointer parameter now defaults to `undefined`, composed via a `child(pointer, segment)` helper that stays `undefined` all the way down once it starts that way, and `MalformedDocumentError`'s message simply omits the location clause when no pointer was threaded in. Every real entry point (`walkAllReferences`, `mergePathsAndComponents`) passes a real root (`#/paths`, `#/components`, ...), so production errors are still fully located; only a test calling these functions directly and by hand gets a pointer-free message.

2. **`getPathItemOperations` (`oas31.ts`) does not thread a pointer at all**, on review advice: it has seven call sites across five otherwise-unrelated files (`paths-and-components.ts`, `reference-walker.ts`, `tag-injection.ts`, `operation-selection.ts`, `prune-components.ts`), none of which otherwise carry pointer context, so full threading there would have rippled far wider than reference-walker.ts's own self-contained recursion for proportionate benefit. It throws `MalformedDocumentError` with just the HTTP method name (`"an Operation Object for HTTP method 'get'"`, no pointer) -- less located than the rest of this proposal's design, but it is the *single* guarded choke point all seven callers now inherit automatically, which is the property that actually matters: before this, a null path item or null operation had to be separately guarded (or missed) at each call site.

3. **Every Group 2/Class C site this proposal identified errors uniformly -- including `securitySchemes`, which §4.1's table flagged as a genuine judgement call rather than resolving.** In review, trying to decide severity per site (does silently treating this null as absent actually risk dropping *reachable* content, or not?) turned out to be exactly the kind of reasoning that produced the `getPathItemOperations` mistake §4.1 itself documents (an early draft would have made it silently filter, rediscovered as wrong before it shipped). Uniform "error, never skip" removes that failure mode by construction for everything this proposal audited.

   One gap this claim does *not* cover, found after implementation rather than designed around: a whole component **bucket** left empty -- `components: { schemas: }`, not one entry inside it -- silently normalises to an empty bucket instead of erroring. `for...in null` is a no-op in JavaScript rather than a throw, so `processComponents`' loop simply never runs and nothing is added; no crash, but a silent drop of whatever the author meant to put there, which is exactly what §3b argues against. Not part of §2.3's original audit (which covered individual entries, not the nine bucket keys themselves) and not fixed here -- flagged for a follow-up rather than reopening this PR's scope.

4. **`renameSecurityRequirements` (`paths-and-components.ts`) moved outside the `oas.components !== undefined` gate and lost its early-return-if-no-renames optimisation.** Neither is in the original design. Both were needed for the same reason: as originally gated, a `security: [ ]` mistake was only ever caught *if* some unrelated scheme elsewhere in the same input happened to collide and need renaming -- otherwise the null flowed straight through to the output untouched. Whether an author's own mistake is caught should not depend on what else the document is merged with; the function now always validates (and, when there is nothing to rename, is a no-op copy).

One thing was designed, then undone on review: `prune-components.ts`'s `securitySchemesInUse` and a matching `catch` around `pruneUnusedComponents` in `index.ts` were written to guard the same null-in-`security` case a second time. Both were dead code -- `pruneUnusedComponents` only ever runs, via `merge()`, on `output`, which is built entirely from pieces `mergePathsAndComponents`'s own `walkAllReferences` call already validated per input; nothing can reach `pruneUnusedComponents` with an unvalidated `null`. Untested, unreachable code is worse than no code, so both were reverted in favour of a comment on `securitySchemesInUse` recording the invariant it now depends on.

No new CLI exit code was added. `merge()`'s new `'malformed-document'` `ErrorType` falls into the CLI's existing `exitCodeForMergeError` default case (`ErrorMerging`, code 3) without any change there, and the CLI's own new catch -- around `discoverExternalDocuments`, for a declared input's content caught during cross-document ref normalisation *before* `merge()` ever runs (§4.4) -- exits the same code for the same reason: both are the same class of authoring problem, just caught at different points in the pipeline.

Coverage on every new and touched line is 100% (`safe-type-checks.ts`, `oas31.ts`, `reference-walker.ts`) or full for the lines actually reachable (`paths-and-components.ts`, `index.ts`); the regression suite covers all 16 originally-designed cases plus four CLI end-to-end tests for the confirmed repros in §2.2, including the "double failure" case (§2.2 Repro 3), now a single clean warning with the ref left unresolved rather than a warning followed by an uncaught crash.

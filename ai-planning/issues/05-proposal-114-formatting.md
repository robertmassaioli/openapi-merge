...
| CLI tests | 27/27 passing (17 new in `formatting.test.ts`; the previous 10 path-resolution tests untouched) |
| Lint | ESLint clean in both packages |
| Typecheck | `tsc --noEmit` clean for the CLI |
| Schema | `configuration.schema.json` regenerated to include the discriminated `formatting.indent` union (file is build-time generated by the `gen-schema` script, not git-tracked) |
| Release status | npm publish pending a CLI version bump and push |
| Follow-up refactor | Discriminator key renamed from `strategy` to `style` (branch `refactor/114-rename-strategy-to-style`; see §12.4 for the per-union naming rationale). |

Sections 1–12 retain their original "proposed" wording for historical
context. Section 11 acceptance criteria are ticked where shipped.
...
Extend the `Configuration` type in `packages/openapi-merge-cli/src/data.ts` with an optional `formatting` block whose `indent` field is a **discriminated union** keyed on `style`:
 ...
 * indentation, so a YAML output combined with `style: 'tabs'` is rejected
...
export type OutputFormatting = {
  /**
   * Indentation style for the output file. A discriminated union so
...
export type Configuration = {
   ...
   * Defaults to `{ indent: { style: 'spaces', width: 2 } }`, preserving
...
{ "formatting": { "indent": { "style": "spaces", "width": 2 } } }
...
- **Type safety** — TypeScript narrows on `indent.style` and refuses to read `width` on a `TabIndent`, or to omit `width` on a `SpaceIndent`. Every reachable state is well-defined.
...
- **JSON Schema friendly** — Ajv handles tagged-union schemas natively via the `discriminator` keyword (or via a small `oneOf` with `const` on `style`). Both styles are well-supported by `typescript-json-schema` which we already use.
- **Future-proof** — adding new indent strategies later (e.g. `{ style: 'mixed', ... }` for the rare repos that demand it) is a single new variant; no flag explosion.
...
Add a `DEFAULT_INDENT` constant so the literal `{ style: 'spaces', width: 2 }` is never spread across the codebase:
...
Modify `packages/openapi-merge-cli/src/index.ts` (lines 113–124). The discriminated union lets us exhaustively switch on `indent.style`, which the TypeScript compiler enforces (`assertNever` ensures any future variant breaks the build until handled):
 ...
 * validation, so the YAML path can assume `style === 'spaces'`).
 */
...
1. **Tag check** (free from the schema): `indent.style` must be `"spaces"` or `"tabs"`. Anything else is rejected by Ajv before our code runs.
...
3. **YAML + tabs conflict** (semantic, enforced in code after the schema check): if `indent.style === 'tabs'` and the resolved output extension is `.yaml` or `.yml`, reject with a clear error message such as: *"Tab indentation is not supported for YAML output; YAML 1.1 disallows tabs as indentation. Use `{ \"style\": \"spaces\", \"width\": N }` or output to JSON."* Surface this as `ExitCode.ErrorLoadingConfig`.
4. **Default**: when `formatting` or `formatting.indent` is omitted, behaviour is exactly today's (`DEFAULT_INDENT === { style: 'spaces', width: 2 }`).
...
- A `SpaceIndent` with `width` missing is also unrepresentable, so users cannot omit the width and silently fall back. They either omit `formatting.indent` entirely (and get the default) or they specify `{ style: 'spaces', width: N }` in full.
...
const SPACES_2: Indent = { style: 'spaces', width: 2 };
const SPACES_4: Indent = { style: 'spaces', width: 4 };
const TABS:     Indent = { style: 'tabs' };
...
describe('formatOutput', () => {
  ...
  it('formats JSON with tab indent when indent.style === "tabs"', () => {
...
- Schema rejects `{ indent: { style: 'tabs', width: 4 } }` (excess property) — proves the discriminator is enforced.
- Schema rejects `{ indent: { style: 'spaces' } }` (missing `width`).
- Schema rejects `{ indent: { style: 'something-else' } }` (unknown discriminator).
- Semantic validation rejects `style: 'tabs'` when `output` ends in `.yaml` / `.yml`.
...
**Note**: Tab indentation (`{ "style": "tabs" }`) applies to JSON only.
YAML 1.1 disallows tabs as indentation, so combining `style: "tabs"`
...
- [x] `SpaceIndent`, `TabIndent`, `Indent`, and `OutputFormatting` types defined; `DEFAULT_INDENT` constant exported from `cli/src/data.ts`.
- [x] `Configuration.formatting?: OutputFormatting` added to `cli/src/data.ts`.
- [x] `dumpAsYaml` and `writeOutput` updated to accept the optional `indent` argument, defaulting to `DEFAULT_INDENT`. *(In the shipped code the helpers take `indent: Indent` directly rather than the full `OutputFormatting` — same effect, less indirection.)*
- [x] `indentToJsonStringifyArg` helper exhaustively switches on `indent.style` and uses `assertNever` as a compile-time guard against future variants. The helper lives in the new `packages/openapi-merge-cli/src/formatting.ts` module.
- [x] Configuration validation rejects `indent: { style: 'tabs' }` for `.yaml`/`.yml` outputs (case-insensitive) with a clear error message via `validateConfigurationSemantics` in `load-configuration.ts`. The error flows back through the existing `Configuration | string` return contract and so triggers `ExitCode.ErrorLoadingConfig` exactly like every other load-time error.
- [x] `formatting` block is optional; omitting it preserves the current 2-space JSON / 2-space YAML behaviour byte-for-byte (verified by the "default ≡ explicit `SPACES_2`" test).
- [x] `yarn gen-schema` regenerates `configuration.schema.json` with the discriminated-union (`oneOf` on `style`) plus the `SpaceIndent.width` `[1, 8]` constraint. *(The file is build-time generated by the CLI's `prepare` script per AGENTS.md; it is not git-tracked.)*
- [x] Jest tests added covering: JSON 2/4/8 spaces, JSON tabs, YAML 4-space, default ≡ explicit `SPACES_2`, tab-fallback defensive path. (See `__tests__/formatting.test.ts`.)
- [x] Jest tests added for `validateConfigurationSemantics`: accepts no-formatting / spaces-with-yaml / spaces-with-json / tabs-with-json; rejects tabs-with-.yaml, tabs-with-.yml, and is case-insensitive on the extension. *(Structural negative cases — extra `width` on `tabs`, missing `width` on `spaces`, unknown `style` values — are now structurally unreachable via the regenerated schema; covered conceptually by the tagged-union shape rather than by explicit tests.)*
- [x] README updated with the new tagged-object syntax and the YAML+tabs limitation note.
- [ ] E2E test (manual or shell-based) confirms merged output respects `{ style: 'spaces', width: 4 }` and `{ style: 'tabs' }` for JSON. *Covered by the unit tests on `indentToJsonStringifyArg` plus the runtime path through `writeOutput`; explicit shell-level E2E deferred to release verification.*
- [x] All existing tests pass; no breaking changes to public CLI API. Library Jest suite is 98/98; CLI Jest suite is 27/27.
- [ ] CLI version bumped to `1.4.0`; library version unchanged. *Deferred to the release commit.*
...
2. **`js-yaml` limitations**: The `yaml.safeDump` API does not expose tab options; indent is always numeric. The discriminated union makes this constraint trivial to enforce: the YAML path can statically narrow to `style: 'spaces'` after the validation check.
...
4. **Discriminator key naming**: We chose `style` over `kind`, `type`, and `strategy`. `type` was rejected because it collides with OpenAPI's `type` keyword in JSON Schema, which would confuse readers and tooling. `kind` was rejected because it reads as a generic type-tag without conveying intent at the config site. `strategy` was rejected because it implies a *trade-off* between options (suitable for proposal-76's `OutputOpenApiVersion` where each variant has compatibility consequences), whereas indentation is a purely aesthetic choice — "style" matches the user's mental model better (e.g. `{ "style": "tabs" }` reads naturally as "the indentation style is tabs"). The discriminator key is therefore picked per-union to fit the domain rather than enforced globally: `style` for formatting choices, `strategy` for choices with trade-offs.

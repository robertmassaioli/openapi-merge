...
- [x] `indentToJsonStringifyArg` helper exhaustively switches on `indent.strategy` and uses `assertNever` as a compile-time guard against future variants. The helper lives in the new `packages/openapi-merge-cli/src/formatting.ts` module.
- [x] Configuration validation rejects `indent: { strategy: 'tabs' }` for `.yaml`/`.yml` outputs (case-insensitive) with a clear error message via `validateConfigurationSemantics` in `load-configuration.ts`. The error flows back through the existing `Configuration | string` return contract and so triggers `ExitCode.ErrorLoadingConfig` exactly like every other load-time error.
...
- [x] `yarn gen-schema` regenerates `configuration.schema.json` with the discriminated-union (`oneOf` on `strategy`) plus the `SpaceIndent.width` `[1, 8]` constraint. *(The file is build-time generated by the CLI's `prepare` script per AGENTS.md; it is not git-tracked.)*
...
- [x] Jest tests added for `validateConfigurationSemantics`: accepts no-formatting / spaces-with-yaml / spaces-with-json / tabs-with-json; rejects tabs-with-.yaml, tabs-with-.yml, and is case-insensitive on the extension. *(Structural negative cases — extra `width` on `tabs`, missing `width` on `spaces`, unknown `strategy` values — are now structurally unreachable via the regenerated schema; covered conceptually by the tagged-union shape rather than by explicit tests.)*
- [x] README updated with the new tagged-object syntax and the YAML+tabs limitation note.
- [ ] E2E test (manual or shell-based) confirms merged output respects `{ strategy: 'spaces', width: 4 }` and `{ strategy: 'tabs' }` for JSON. *Covered by the unit tests on `indentToJsonStringifyArg` plus the runtime path through `writeOutput`; explicit shell-level E2E deferred to release verification.*
